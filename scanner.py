...
Scans top 30 coins every hour, emits signals via callback
"""
...
_running   = False
_thread    = None
_on_signal = None   # callback(signal_dict)
_coin_list = []
...
def _scan_once():
    ...
    if not coins:
        print("⚠️  Could not fetch coin list"); _last_scan_info["status"] = "error"; return
    _coin_list = coins
    _last_scan_info["coins"] = coins
    print(f"📋 Top {len(coins)} coins: {', '.join(coins[:10])}...")

    notifier.send(
        f"🔍 *Scanner running* — {_lk_now()}\n"
        f"Scanning *{len(coins)} coins*\n"
        f"Balance: `${tm.get_state()['balance']:.2f}`"
    )
    ...
    for coin in coins:
        ...
        try:
            ...
            # Use last 3 completed candles as signal candidates
            targets = list(range(max(100, len(df_ind)-4), len(df_ind)-1))
            ...
            for i in targets:
                ...
                for direction in directions:
                    if i - last_sig[direction] < config.COOLDOWN_BARS: continue
                    ...
                    if alloc_usd < config.MIN_TRADE_USD: continue
                    ...
                    signals_found += 1
                    print(f"  {'✅' if is_win else '❌'} {coin} {direction} score={score} → {outcome} ${pnl_usd:+.2f}")
